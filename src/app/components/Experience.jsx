...
const ExperienceCard = ({ job, index }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative flex flex-col md:flex-row items-start gap-6 sm:gap-8 md:gap-12 py-10 sm:py-12 border-b border-slate-100 last:border-0"
        >
                    ...
                    {job.url ? (
                        ...
                        >
                            ...
                            <div className="flex flex-col">
                                <h3 className="flex items-center gap-2 text-xl sm:text-2xl font-bold text-slate-800 tracking-tight transition-colors duration-300 group-hover/company:text-blue-600">
                                    {job.company}
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <h3 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors">
                                {job.company}
                            </h3>
                        </div>
                    )}

                    <div className="flex flex-col gap-2 text-sm font-medium text-slate-500 mt-2">
            ...
            {/* Right Side: Role, Highlights, Tech */}
                ...
                <div className="relative bg-white lg:bg-transparent lg:hover:bg-white p-0 lg:p-8 lg:-m-8 rounded-[2rem] lg:border border-transparent lg:hover:border-slate-100 lg:hover:shadow-2xl lg:hover:shadow-blue-900/5 transition-all duration-500">
                    ...
                    <ul className="mb-8 space-y-3 sm:space-y-4">
                        {job.highlights.map((point, idx) => (
                            <li key={idx} className="flex items-start gap-3 sm:gap-4 text-slate-600 text-sm sm:text-base leading-relaxed">
                                <span className="mt-1.5 shrink-0 bg-blue-50 text-blue-500 p-1 rounded-full group-hover:bg-blue-500 group-hover:text-white transition-colors duration-300" aria-hidden="true">
